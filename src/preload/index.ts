...
const api: MsgViewerApi & { openDroppedFile(file: File): Promise<LoadResult> } = {
  ...
  associateTypes: (exts) => ipcRenderer.send('associate-types', exts),
  checkAssociation: () => ipcRenderer.invoke('check-association'),
  dismissAssociationOffer: () => ipcRenderer.send('dismiss-association-offer'),
  ...
  startAttachmentDrag: (attachmentId) => ipcRenderer.send('attachment-drag', attachmentId),
  onToast: (cb) => {
